...
st.title('Simple Streamlit App2')
...
with col1:
    st.subheader('Generacion de images a partir de texto')
    ...
    if st.button('Generar imagen'):
        ...
        st.image(result_link, caption='Imagen Mostrada', use_column_width=True)
        #st.session_state['imagen_generada'] = result_link
        st.write('Clasificación de la imagen generada por la IA')
    if st.button('Clasificar imagen generada'):
        ...
        st.image(result_link_estado, caption='Imagen Mostrada', use_column_width=True)
...
with col2:
    st.subheader('Clasificación de imagenes')
